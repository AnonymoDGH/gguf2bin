...
int tok_from_gguf(GGUF *g, Tokenizer *t){
  ...
  for(i32 i=0;i<(i32)nm;i++){ char *s=mg[i]; char *sp=strchr(s,' '); if(!sp) continue; size_t al=(size_t)(sp-s), bl=strlen(sp+1); char *key=malloc(al+1+bl+1); if(!key) continue; memcpy(key,s,al); key[al]=1; memcpy(key+al+1,sp+1,bl); key[al+1+bl]=0; shash_put(t->merges,key,i); free(key); }
  return 0;
}
...
int tok_read_section(FILE *f, Tokenizer *t){
  ...
  if(!t->tok) return -1;
  for(i32 i=0;i<t->n;i++){ u32 l; if(fread(&l,4,1,f)!=1) goto fail; char *s=malloc(l+1); if(!s) goto fail; if(fread(s,1,l,f)!=l){ free(s); goto fail; } s[l]=0; t->tok[i]=s; }
  t->mergestr=calloc(t->nmerges?(size_t)t->nmerges:1,sizeof(char*));
  if(!t->mergestr) goto fail;
  for(i32 i=0;i<t->nmerges;i++){ u32 l; if(fread(&l,4,1,f)!=1) goto fail; char *s=malloc(l+1); if(!s) goto fail; if(fread(s,1,l,f)!=l){ free(s); goto fail; } s[l]=0; t->mergestr[i]=s; }
  shash_init(t->vocab,1u<<19); for(i32 i=0;i<t->n;i++) shash_put(t->vocab,t->tok[i],i);
  shash_init(t->merges,1u<<19);
  for(i32 i=0;i<t->nmerges;i++){ char *s=t->mergestr[i]; char *sp=strchr(s,' '); if(!sp) continue; size_t al=(size_t)(sp-s), bl=strlen(sp+1); char *key=malloc(al+1+bl+1); if(!key) continue; memcpy(key,s,al); key[al]=1; memcpy(key+al+1,sp+1,bl); key[al+1+bl]=0; shash_put(t->merges,key,i); free(key); }
  return 0;
fail:
  tok_free(t); return -1;
}
