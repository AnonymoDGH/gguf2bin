...
static void shash_init(SHash *h, u32 cap){
  if(cap < 16) cap = 16;
  /* power of two */
...
int tok_from_gguf(GGUF *g, Tokenizer *t){
  ...
  t->vocab=malloc(sizeof(SHash)); t->merges=malloc(sizeof(SHash));
  shash_init(t->vocab,1u<<19); for(i32 i=0;i<t->n;i++) shash_put(t->vocab,t->tok[i],i);
  shash_init(t->merges,1u<<19); t->mergestr=mg; t->nmerges=(i32)nm;
  for(i32 i=0;i<(i32)nm;i++){ char *s=mg[i]; char *sp=strchr(s,' '); if(!sp) continue; size_t al=(size_t)(sp-s), bl=strlen(sp+1); char *key=malloc(al+1+bl+1); memcpy(key,s,al); key[al]=1; memcpy(key+al+1,sp+1,bl); key[al+1+bl]=0; shash_put(t->merges,key,i); free(key); }
  return 0;
}
...
int tok_read_section(FILE *f, Tokenizer *t){
  ...
  t->vocab=malloc(sizeof(SHash)); t->merges=malloc(sizeof(SHash));
  u32 nv,nm; i32 b,e,u;
  if(fread(&nv,4,1,f)!=1||fread(&nm,4,1,f)!=1||fread(&b,4,1,f)!=1||fread(&e,4,1,f)!=1||fread(&u,4,1,f)!=1) return -1;
  t->n=(i32)nv; t->nmerges=(i32)nm; t->bos=b; t->eos=e; t->unk=u?u:0;
  t->tok=calloc(t->n?t->n:1,sizeof(char*));
  for(i32 i=0;i<t->n;i++){ u32 l; if(fread(&l,4,1,f)!=1) return -1; char *s=malloc(l+1); if(fread(s,1,l,f)!=l){ free(s); return -1; } s[l]=0; t->tok[i]=s; }
  t->mergestr=calloc(t->nmerges?t->nmerges:1,sizeof(char*));
  for(i32 i=0;i<t->nmerges;i++){ u32 l; if(fread(&l,4,1,f)!=1) return -1; char *s=malloc(l+1); if(fread(s,1,l,f)!=l){ free(s); return -1; } s[l]=0; t->mergestr[i]=s; }
  shash_init(t->vocab,1u<<19); for(i32 i=0;i<t->n;i++) shash_put(t->vocab,t->tok[i],i);
  shash_init(t->merges,1u<<19);
  for(i32 i=0;i<t->nmerges;i++){ char *s=t->mergestr[i]; char *sp=strchr(s,' '); if(!sp) continue; size_t al=(size_t)(sp-s), bl=strlen(sp+1); char *key=malloc(al+1+bl+1); memcpy(key,s,al); key[al]=1; memcpy(key+al+1,sp+1,bl); key[al+1+bl]=0; shash_put(t->merges,key,i); free(key); }
  return 0;
}
