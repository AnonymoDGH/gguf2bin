...
public class MainActivity extends Activity {
    ...
    private void refresh() {
        display.clear(); files.clear();
        File[] fs = getFilesDir().listFiles();
        if (fs != null) for (File f : fs)
            if (f.getName().endsWith(".g2bx")) {
                files.add(f);
                display.add(String.format("%s  ·  %.0f MB", f.getName(), f.length() / 1048576.0));
            }
        if (adapter != null) adapter.notifyDataSetChanged();
        if (display.isEmpty()) status.setText("Sin modelos. Pega una URL abajo.");
        else status.setText(display.size() + " modelo(s) listo(s). Toca uno para chatear.");
    }
    ...
    private void download() {
        ...
        new Thread(() -> {
            try {
                ...
                runOnUiThread(() -> { progress.setIndeterminate(total <= 0); if(total>0) progress.setMax(100); });
                while ((r = in.read(buf)) > 0) {
                    ...
                    runOnUiThread(() -> {
                        if (total > 0) {
                            ...
                            status.setText(String.format("%d%%  %.1f/%.1f MB", pct, got[0]/1048576.0, total/1048576.0));
                        } else status.setText(String.format("%.1f MB", got[0]/1048576.0));
                    });
                }
                out.close(); in.close();
                runOnUiThread(() -> {
                    status.setText("Descargado: " + fName); progress.setVisibility(View.GONE);
