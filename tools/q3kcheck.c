/* q3kcheck.c — valida matmul_q3_K[_b] (dot entero AVX2 con act Q8).
 * Doble referencia (misma entrada Q8 escalar en ambas):
 *  A) dequant escalar punteado con la activación Q8 reconstruida: tol 1e-3.
 *  B) dequant escalar punteado con x float (verdad GGML): tol 15%% + 2%%abs.
...
static void check(int n,int d,int B){
  ...
  { u16 dh=f32_to_half(0.5f); u8 *p=w;
    for(size_t k=0;k<(size_t)d*nb;k++){
      for(int j=0;j<32;j++) *p++=(u8)((k*131+7+j*37)&0xff);
      for(int j=0;j<64;j++) *p++=(u8)((k*57+j*11)&0xff);
      for(int j=0;j<12;j++) *p++=(u8)((k*13+j*7)&0xff);
      memcpy(p,&dh,2); p+=2;
    } }
  ...
  if(B==1) matmul_q3_K(got,x,w,n,d);
  else matmul_q3_K_b(got,x,w,n,d,B);
  for(int t=0;t<B;t++) for(int i=0;i<d;i++){
    gguf_dequant(T_Q3_K, w+(size_t)i*nb*110, tmp, (u64)n);
...
int main(int argc, char **argv){
  if(argc>1 && !strcmp(argv[1],"onehot")){
    /* one-hot barriendo los 16 grupos: got[i] debe ser deq[i][p] exacto */
    int n=256,d=2;
    u8 *w=malloc(110*(size_t)d);
    f32 *x=calloc((size_t)n,sizeof(f32));
    ...
    for(int p=0;p<n;p++){
      memset(x,0,(size_t)n*sizeof(f32)); x[p]=1.f;
      memset(got,0,(size_t)d*sizeof(f32));
      matmul_q3_K(got,x,w,n,d);
      int bad=0; f32 g0=0,t0=0;
      for(int i=0;i<d;i++){
        gguf_dequant(T_Q3_K, w+(size_t)i*110, tmp, (u64)n);
        if(i==0){ g0=got[0]; t0=tmp[p]; }
        if(fabsf(got[i]-tmp[p])>1e-4) bad++;
      }
      if(bad) printf("pos %3d (g=%d j=%d): DIVERGE got0=%f deq0=%f\n",
        p,p/16,p%16,g0,t0);
    }
    printf("barrido one-hot completo\n");
    return 0;
  }
  if(argc>2){ check(atoi(argv[1]),atoi(argv[2]),argc>3?atoi(argv[3]):1); }
  else { check(512,64,1); check(512,64,4); check(5120,64,1); }
  printf(fails?"q3kcheck: FAIL\n":"q3kcheck: OK\n");
  return fails?1:0;
}
